//加密密码
function chkpwd(stuId, pwd) {
	if (pwd != '') {
		var s = md5(stuId + md5(pwd).substring(0, 30).toUpperCase() + '10822')
...
function get_txt_mm_expression(pwd) {
	var result = 0;
...
//获取txt_mm_userzh(userzh为学号)
function get_txt_mm_userzh(stuId, pwd) {
	var inuserzh = "0";
	if (pwd.toLowerCase().trim().indexOf(stuId.toLowerCase().trim()) > -1) {
		inuserzh = "1";
	}
	return inuserzh;
}
	...
	if (str != '') {
		...
		for (var i = 1; i < arr.length - 1; i++) {
			...
			if (((thirdIndex - secondIndex == 1) && (secondIndex - firstIndex == 1))
					|| (thirdIndex == secondIndex && secondIndex == firstIndex)) {
...
function chkyzm(obj) {
	if (obj != '') {
		var s = md5(
				md5(obj.toUpperCase()).substring(0, 30).toUpperCase() + '10822')
				.substring(0, 30).toUpperCase();
